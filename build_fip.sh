...
atf_repo_url=${atf_repo_url:-https://review.trustedfirmware.org/TF-A/trusted-firmware-a.git}
...
# Update all repos
update_repos() {
    update_repo u-boot.git "${uboot_repo_url}" "${uboot_branch}"
    update_repo atf.git "${atf_repo_url}" "${atf_branch}"
}
...
prepare_atf() {
    rm -rf atf
    mkdir atf
    git --git-dir rkbin.git --work-tree atf checkout -f master
